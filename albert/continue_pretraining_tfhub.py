...
def create_tokenizer_from_hub_module(albert_module):
  """Get the vocab file and casing info from the Hub module."""
  tokenization_info = albert_module(signature="tokenization_info", as_dict=True)
  with tf.Session() as sess:
    vocab_file, do_lower_case = sess.run([tokenization_info["vocab_file"], tokenization_info["do_lower_case"]])
  return tokenization.FullTokenizer(vocab_file=vocab_file, do_lower_case=do_lower_case, spm_model_file=vocab_file)


def model_fn_builder(albert_hub_module_handle, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    albert_module = hub.Module(albert_hub_module_handle, tags=tags, trainable=True)
    tok = create_tokenizer_from_hub_module(albert_module)
    vocab_size = len(tok.vocab)

    albert_mlm_inputs = dict(
      input_ids=input_ids,
      input_mask=input_mask,
      segment_ids=segment_ids,
      mlm_positions=masked_lm_positions)
    albert_outputs = albert_module(albert_mlm_inputs, signature="mlm", as_dict=True)

    (masked_lm_loss, masked_lm_example_loss,
     masked_lm_log_probs) = get_masked_lm_output(albert_outputs["sequence_output"],
                                                 albert_outputs["mlm_logits"],
                                                 vocab_size,
                                                 masked_lm_positions,
                                                 masked_lm_ids,
                                                 masked_lm_weights)
    ...
    (sentence_order_loss, sentence_order_example_loss,
     sentence_order_log_probs) = get_sentence_order_output(albert_outputs["pooled_output"],
...
def get_masked_lm_output(sequence_output, mlm_logits, vocab_size, positions, label_ids, label_weights):
